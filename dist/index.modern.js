import React, { useReducer, useCallback, useMemo, Fragment as Fragment$1, useEffect } from 'react';
...
var ReactDataTableBootstrap = function ReactDataTableBootstrap(_ref) {
  ...
  var render = useCallback(function () {
    ...
    rows.filter(function (r) {
      ...
      for (var key in r) {
        ...
        if ((_r$key = r[key]) !== null && _r$key !== void 0 && _r$key.toString().toUpperCase().includes(store.textFilter.toUpperCase())) {
          has = true;
          break;
        }
      }
      ...
      var _loop2 = function _loop2(h) {
        ...
        if ((_headParams$h = headParams[h]) !== null && _headParams$h !== void 0 && _headParams$h.classBodyTr) {
      ...
      for (var h in headParams) {
        _loop2(h);
      }

      arrRowsRender.push( /*#__PURE__*/React.createElement("tr", tr, React.Children.toArray(row)));
    });
  ...
  var getHeader = useMemo(function () {
    return /*#__PURE__*/React.createElement("thead", null, React.Children.toArray(head.map(function (tr, j) {
      return /*#__PURE__*/React.createElement(Fragment$1, null, /*#__PURE__*/React.createElement("tr", null, React.Children.toArray(tr.map(function (h) {
        h.sort = h.sort !== false;
  ...
  var getLegend = useMemo(function () {
    return /*#__PURE__*/React.createElement(Fragment$1, null, "Mostrando ", !store.rowsRender.length ? 0 : (store.numberPage - 1) * store.pageLength + 1, " a ", store.rowsRender.slice(store.numberPage === 1 ? 0 : (store.numberPage - 1) * store.pageLength, store.pageLength * store.numberPage).length - store.pageLength === 0 ? store.pageLength * store.numberPage : store.rowsRender.slice(0, store.pageLength * store.numberPage).length, " de ", store.rowsRenderFull.length !== store.rowsRender.length ? store.rowsRender.length + ' filtrados de ' : '', !isProcessing ? rows.length : 0, " registros");
